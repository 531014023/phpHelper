...
class TimeHelper
{
     ...
     */
    static function format_time( $time = null, $format='Y-m-d H:i:s')
    {
     ...
     * @return array
     */
    static function get_date_from_range( $startdate,  $enddate)
    {
     ...
     * 获取最近N天所有日期
     */
    static function get_date_from_days($days,$time = '', $format='Y-m-d'){
        $time = $time != '' ? $time : time();
        //组合数据
        $date = [];
        for ($i=1; $i<=$days; $i++){
            $date[] = date($format ,strtotime( '+' . $i-$days .' days', $time));
        }
        return $date;
    }
